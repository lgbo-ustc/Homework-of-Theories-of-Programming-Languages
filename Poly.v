...
Definition partition {X : Type} (test : X -> bool) (l : list X) 
                     : list X * list X :=
(* FILL IN HERE *) 
((filter test l),(filter (negf test) l)).
...
Lemma map_rev_auxi:forall (X Y :Type) (f:X->Y) (l:list X) (x:X),
...
induction l.
reflexivity.
simpl. rewrite <-IHl. rewrite map_rev_auxi. reflexivity.
Qed.
...
simpl. intro H;inversion H.
simpl. intro H. apply eq_remove_S.  apply IHm'. apply H.
Qed.
(** [] *)
...
intro m; destruct m as [|m'].
intro H; inversion H.
intro H. apply eq_remove_S. apply IHn'.
inversion H.
...
induction l'.
reflexivity.
assert (split (x0::l')=((fst x0)::(fst (split l')),(snd x0)::(snd (split l')))).
destruct x0.
simpl. reflexivity.
rewrite H0.
simpl. reflexivity.
...
(** Write down a theorem in Coq stating that [fold_map] is correct,
    and prove it. *)
Theorem fold_map_correct:forall {X Y:Type} (l:list X) (f:X->Y),fold_map f l=map f l.
intros X Y l f.
induction l.
reflexivity.
simpl.
...
match l with
|nil => false
|x::nil => if f x then true else false
|x1::l' => if f x1 then  andb true (forallb f l') else false
end.
Example forallb1: forallb  oddb [1,3,5,7,9]=true.
Proof. reflexivity. Qed.
Example forallb2:forallb evenb [0,2,4,5]=false.
Proof. reflexivity. Qed.
...
Example existsb1:existsb (andb true) [true,true,false] = true.
Proof. reflexivity. Qed.
Example existsb2:existsb oddb [1,0,0,0,0,3] = true.
Proof. reflexivity. Qed.
Example existsb3:existsb evenb [] = false.
Proof. reflexivity. Qed.
...
|nil => false
|_::_=>
if forallb ((fun (f1:X->bool) (x:X)=> if f1 x then false else true)f) l then false else true
end. 
Example existsb'1:existsb' oddb [1,0,0,3] = true.
Proof. unfold existsb'. simpl. reflexivity. Qed.
Theorem eq_existsb_existsb':forall {X:Type} (f:X->bool) (l:list X),existsb f l=existsb' f l.
Proof.
intros X f l.
induction l.
simpl.
reflexivity.
assert (existsb f (x::l)= orb (f x) (existsb f l)).
simpl.
...
(* FILL IN HERE *)
(1)length l=n=0时,有index X 1 nil = None,命题成立;
(2)设当length l = k时，也满足 index X (S k) l = None
(3)当length l= k+1时，设l=h::l'.
则 index X S(S k) x::l' 归约为 index (pred S(S k)) l'=index (S k) l'.
由(2)可知 index (S k) l' = None,所以 index X S(S k) l =None.
即证 forall X n l, length l = n -> @index X (S n) l = None.
*)
(** [] *)
