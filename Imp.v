...
Proof.
intro.
bexp_cases (induction b) Case;try (simpl;reflexivity;unfold optimize_0plus_b).
unfold optimize_0plus_b. simpl.
assert (aeval (optimize_0plus a) =aeval a).
apply optimize_0plus_sound.
assert (aeval (optimize_0plus a0)=aeval a0).  apply optimize_0plus_sound with (e:=a0).
...
assert (aeval (optimize_0plus a0)=aeval a0).  apply optimize_0plus_sound with (e:=a0).
rewrite H. rewrite H0. reflexivity.
bexp_cases (induction b) SCase;try(simpl;reflexivity).
simpl. simpl in IHb. rewrite IHb. reflexivity.
simpl. simpl in IHb. rewrite IHb. reflexivity.
simpl. simpl in IHb. simpl in IHb0. rewrite IHb. reflexivity.
simpl. simpl in IHb. rewrite IHb. reflexivity.
...
Qed.
Fixpoint optimize_plus0 (e:aexp) : aexp := 
  match e with
  | ANum n => 
      ANum n
  | APlus e1 (ANum 0) => 
...
Theorem optimize_plus0_sound:forall e,aeval (optimize_plus0 e) = aeval e.
Proof.
intros.
...
Case "APlus". 
    aexp_cases (destruct e2) SCase; 
      try (simpl; simpl in IHe1; rewrite IHe1; rewrite IHe2; reflexivity).
    SCase "ANum". destruct n.
      simpl; rewrite <- IHe1; rewrite plus_comm;reflexivity.  
simpl. rewrite <- IHe1. reflexivity. simpl. simpl in IHe2.  rewrite IHe2. rewrite <- IHe1. reflexivity.
simpl. simpl in IHe2. rewrite IHe2. rewrite <- IHe1. reflexivity.
simpl. simpl in IHe2. rewrite IHe2. rewrite <- IHe1. reflexivity.
simpl. simpl in IHe2. rewrite IHe2. rewrite <- IHe1. reflexivity.
simpl. simpl in IHe2. rewrite IHe2. rewrite <- IHe1. reflexivity.
Qed.
...
|E_BEq : forall (a1 a2:aexp) , bevalR (BEq a1 a2) (beval (BEq a1 a2))
|E_BLe : forall (a1 a2:aexp) , bevalR (BLe a1 a2) (beval (BLe a1 a2))
|E_BNot : forall (B:bexp) (b:bool),bevalR B b -> bevalR (BNot B) (negb b)
|E_BAnd : forall (B1 B2:bexp) (b1 b2:bool) , bevalR B1 b1 -> bevalR B2 b2 -> bevalR (BAnd B1 B2) (b1&&b2)
.
Theorem beval_iff_bevalR:forall B b,bevalR B b <-> beval B =b.
Proof.
split.
intros;induction H;subst;reflexivity.
generalize dependent b.
induction B;simpl;intros;subst; constructor;try apply IHB. reflexivity. 
...
Print beq_nat_false.
assert (n<>n0).
intro H1.  contradiction H.  rewrite H1;reflexivity.
...
destruct X.
unfold update.
remember (beq_id (Id n0) (Id n0)) as E.
destruct E.
reflexivity.
unfold beq_id in HeqE.
...
unfold update.
remember (beq_id (Id n0) (Id n)) as E.
destruct E.
reflexivity.
reflexivity.
Qed.
...
destruct k1;destruct k2.
unfold update.
remember (beq_id (Id n) (Id n0)) as E.
destruct E.
assert (n=n0).
apply beq_nat_eq.
apply  HeqE.
subst n0. rewrite H. reflexivity.
reflexivity.
Qed.
(** [] *)
...
unfold update.
remember (beq_id (Id n) (Id n1)) as E1.
destruct E1.
assert (n=n1).
unfold beq_id in HeqE1.
apply beq_nat_eq. assumption.
rewrite <- H0.
...
inversion Heqloopdef.  rewrite H1 in IHcontra2. apply IHcontra2. rewrite H2. reflexivity.
Qed.
(** [] *)
...
|R_Ass : forall (X:id) n,no_whilesR (X::=n) 
|R_Seq : forall (c1 c2:com),no_whilesR(c1)->no_whilesR(c2)->no_whilesR(c1;c2)
|R_If : forall (ct cf:com) (b:bexp),no_whilesR ct -> no_whilesR cf -> no_whilesR(IFB b THEN ct ELSE cf FI).
...
intro. apply R_Ass.
simpl.  intro.
apply R_Seq. apply IHc1.   apply andb_true_elim1 in H. assumption.
apply IHc2. apply andb_true_elim2 in H. assumption.
simpl. intro. apply R_If. apply IHc1. apply andb_true_elim1 in H. assumption.
apply IHc2. apply andb_true_elim2 in H. assumption.
simpl. intro. inversion H.
induction c.
intro. simpl. reflexivity.
simpl. intro. reflexivity. simpl. intro. assert (no_whiles c1 =true).
apply IHc1. inversion H. apply H2. assert (no_whiles c2 = true).
inversion H. apply IHc2. apply H4. rewrite H0. rewrite H1. reflexivity.
simpl. intro. inversion H. assert(no_whiles c1 =true). apply IHc1. apply H2.
assert (no_whiles c2 =true). apply IHc2. assumption. rewrite H5. rewrite H6. reflexivity.
intro. inversion H.
Qed.
(** [] *)
...
(* FILL IN HERE *)
Theorem no_whiles_terminating:forall  (c:com),(no_whiles c=true) ->forall st:state,exists st':state,c/st||st'.
induction c.
intros. 
exists (st). 
apply E_Skip.
...
intros.
simpl in  H.
assert (no_whiles c1 = true). apply andb_true_elim1 in H. assumption.
assert (no_whiles c2 = true). apply andb_true_elim2 in H. assumption.
destruct IHc1 with (st:=st).
assumption.
destruct IHc2 with (st:=x). assumption.
exists x0. assert (c1/st||x->c2/x||x0->(c1;c2)/st||x0).
apply E_Seq. apply H4. assumption. assumption.
intros. simpl in H. 
assert (no_whiles c1 = true). apply andb_true_elim1 in H. assumption.
assert (no_whiles c2 = true). apply andb_true_elim2 in H. assumption.
remember (beval st b) as E.
destruct E.
 destruct IHc1 with (st:=st). assumption. exists x. symmetry in HeqE.
apply E_IfTrue. assumption. assumption.
destruct IHc2 with (st:=st). assumption. exists x. symmetry in HeqE. apply E_IfFalse.
...
Print subtract_slowly_body. Print subtract_slowly.
(* FILL IN HERE *)
Theorem ss_body_preserves_invariant: forall st st' x z,
ss_invariant x z st -> st X <> 0 ->
subtract_slowly_body/st||st' ->
ss_invariant x z st'.
Proof.
unfold ss_invariant,fact_body.
intros st st' x z Hm HZnz He.
inversion He;subst;clear He.
inversion H1;subst;clear H1.
inversion H4;subst;clear H4.
unfold update . simpl.
...
simpl.  destruct (st X) as [|x'].
simpl. rewrite <- Hm. simpl. contradiction HZnz. reflexivity.
rewrite <- Hm.
...
ss_invariant x z st'.
Proof.
intros st st' x z H Hce.
remember (subtract_slowly) as c.
ceval_cases (induction Hce) Case; 
              inversion Heqc; subst; clear Heqc.
assumption.
apply IHHce2.  apply ss_body_preserves_invariant with st. assumption.
intros Contra. simpl in H0;subst.  rewrite Contra in H0. inversion H0.
assumption. reflexivity.
Qed.
Theorem ss_correct:forall st st' x z,
st X=x->st Z=z->
subtract_slowly/st||st'->
st' Z = z-x.
Proof.
intros st st' x z HX HZ Hce.
assert (ss_invariant x z st).
subst. unfold ss_invariant. reflexivity.
assert (ss_invariant x z st').
apply ss_loop_preserves_invariant with st.  assumption. 
assumption. unfold ss_invariant in H0. 
apply guard_false_after_loop in Hce. simpl in Hce.
destruct (st' X). rewrite <- H0. omega.
simpl in Hce. inversion Hce.
Qed.
(** [] *)
...
(c1;(CWhile b (c3;c2))).
Reserved Notation "c1 '/' st '||' st'" (at level 40, st at level 39).
Inductive ceval_for : com -> state -> state -> Prop :=
  | E_Skip' : forall st,
      ceval_for CSkip  st  st
  | E_Ass'  : forall st a1 n X,
      aeval st a1 = n ->
      ceval_for (X ::= a1)  st  (update st X n)
  | E_Seq' : forall c1 c2 st st' st'',
      ceval_for c1  st   st' ->
      ceval_for c2  st'  st'' ->
      ceval_for (c1 ; c2)  st  st''
  | E_IfTrue' : forall st st' b1 c1 c2,
      beval st b1 = true ->
      ceval_for c1  st  st' ->
      ceval_for (IFB b1 THEN c1 ELSE c2 FI)  st  st'
  | E_IfFalse' : forall st st' b1 c1 c2,
      beval st b1 = false ->
      ceval_for c2  st  st' ->
      ceval_for (IFB b1 THEN c1 ELSE c2 FI)  st  st'
  | E_WhileEnd' : forall b1 st c1,
      beval st b1 = false ->
      ceval_for (WHILE b1 DO c1 END)  st  st
  | E_WhileLoop' : forall st st' st'' b1 c1,
      beval st b1 = true ->
      ceval_for c1  st  st' ->
      ceval_for (WHILE b1 DO c1 END)  st'  st'' ->
      ceval_for (WHILE b1 DO c1 END)  st  st''
  | E_ForEnd : forall b st1 st2 c1 c2 c3,ceval_for c1 st1 st2->beval st2 b = false ->
                     ceval_for (FOR c1 , b , c2 DO c3 END) st1 st2
  | E_ForLoop : forall b st1 st2 st3 st4 c1 c2 c3,ceval_for c1 st1 st2->beval  st2 b=true ->
                       ceval_for (c3;c2) st2 st3 -> ceval_for (FOR c1 , b ,c2 DO c3 END) st3 st4 ->
                       ceval_for (FOR c1 , b, c2 DO c3 END) st1  st4.
...
Theorem beval'_iff_beval:forall st e,beval' st e = beval st e.
Proof.
intros.
...
match prog with
| nil => stack
| (SPush x)::prog' => s_execute st (x::stack) prog'
| (SLoad X)::prog' => s_execute st ((st X)::stack) prog'
| SPlus::prog' => match stack with
                          | s1::(s2::stack') => s_execute st ((s2+s1)::stack') prog'
                          | _ => nil
                          end
...
Theorem execute_eq1 : forall (e : aexp) (st : state) (s1 : list nat) (s2 : list sinstr),
s_execute st s1 (s_compile e ++ s2) =s_execute st ((aeval st e) :: s1) s2.
Proof.
induction e;try(simpl;reflexivity);try simpl.
intros.
assert ((s_compile e1 ++ s_compile e2 ++ [SPlus]) ++ s2=s_compile e1 ++ s_compile e2 ++ [SPlus] ++ s2).
repeat rewrite  app_ass. reflexivity.
rewrite H.   simpl.
assert (s_execute st s1 (s_compile e1 ++ s_compile e2 ++ SPlus :: s2)=
           s_execute st  ((aeval  st e1) :: s1) (s_compile e2++SPlus::s2)).
rewrite IHe1. reflexivity. rewrite H0. rewrite IHe2. simpl. 
reflexivity.
intros.
assert ((s_compile e1 ++ s_compile e2 ++ [SMinus]) ++ s2=s_compile e1 ++ s_compile e2 ++ [SMinus] ++ s2).
repeat rewrite  app_ass. reflexivity.
rewrite H.
assert (s_execute st s1 (s_compile e1 ++ s_compile e2 ++ SMinus :: s2)=
           s_execute st  ((aeval  st e1) :: s1) (s_compile e2++SMinus::s2)).
rewrite IHe1. reflexivity. simpl.
rewrite H0. rewrite IHe2. simpl. reflexivity.
intros.
assert ((s_compile e1 ++ s_compile e2 ++ [SMult]) ++ s2=s_compile e1 ++ s_compile e2 ++ [SMult] ++ s2).
repeat rewrite  app_ass. reflexivity.
rewrite H.
assert (s_execute st s1 (s_compile e1 ++ s_compile e2 ++ SMult :: s2)=
           s_execute st  ((aeval  st e1) :: s1) (s_compile e2++SMult::s2)).
rewrite IHe1. reflexivity. simpl.
rewrite H0. rewrite IHe2. simpl. 
 reflexivity.
Qed.

Theorem s_exe_eq2:forall st e,
s_execute st [] (s_compile e)=s_execute st [aeval st e] [].
Proof.
intros.
induction e;try(simpl;reflexivity);
try (simpl;rewrite execute_eq1;rewrite execute_eq1; simpl;reflexivity).
Qed.
